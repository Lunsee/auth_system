...
DB_HOST = os.getenv("DB_HOST", "db") # для DOCKER
...
def reset_database():
    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("DROP TABLE IF EXISTS website_users CASCADE;"))
        conn.execute(text("DROP TABLE IF EXISTS refresh_tokens CASCADE;"))
        conn.execute(text("DROP TABLE IF EXISTS user_role CASCADE;"))
